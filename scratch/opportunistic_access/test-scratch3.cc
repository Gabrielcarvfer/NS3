//
// Created by  Gabriel Ferreira (@gabrielcarvfer) on 23-Jan-19.
//
...
#include <ns3/contrib-sauloDaMata-module.h>
...
// Natalândia   66000, 94000, 0
    ...
    setup_mobility2(&ueNodesNatalandia, "ns3::ConstantPositionMobilityModel", 58000.0, 50000.0, 0.0, 3000.0);
    ...
    echoClient2.SetAttribute("Interval", TimeValue(MilliSeconds(100)));
    ...
    for (NodeContainer::Iterator i = ueNodesUnai.Begin(); i != ueNodesUnai.End(); ++i)
    {
    ...
    //VOIP
    VoipServerHelper voipServer (serverPort+2);
    voipServer.SetAttribute("Interval", TimeValue(MilliSeconds(10)));
    serverApps.Add(voipServer.Install(remoteHost));

    VoipClientHelper voipClient(serverAddress, serverPort+2);
    clientApps.Add(voipClient.Install(allUeNodes));
