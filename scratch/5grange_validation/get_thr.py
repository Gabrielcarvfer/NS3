...
def get_thr(deleteFiles=False):
    ...
    if deleteFiles:
        import os
        os.remove(local+arq_p2p_remote)
        os.remove(local+arq_p2p_enb)
        os.remove(local+arq_app)
        os.remove(local+arq_phy)
        os.remove(local+"5grange_phy_trace_lte.txt")


if __name__ == "__main__":
    get_thr(deleteFiles=True)