...
def plot_simulation_topology(baseDir, simulationModel):
    ...
    for ue in simulationModel["UE"].values():
        ...
        ax1.scatter(*pos, color="blue", marker="s", label="UE%d"%i)
        ...
        #ax.add_artist(ab)
    ax.annotate("UE%d"%i, numpy.subtract(pos, annotationPosition))
    ax1.annotate("UE%d"%i, numpy.subtract(pos, annotationPosition))
    ...
    for eNB in simulationModel["eNB"].values():
        ...
        ax1.scatter(*pos, color="purple", marker="^", linewidth=8, label="gNB%d"%i)
        ax1.annotate("eNB%d"%i, numpy.subtract(pos,annotationPosition))
    ...
    c1 = plt.Circle((50, 50), 50.0, alpha=0.2, linewidth=5, fill=False)#,fill=False)
    ...
    plt.tight_layout()
    fig.savefig(baseDir + os.sep + "topology_with_pu.png")
    fig1.savefig(baseDir + os.sep + "topology_without_pu.png")
...
if __name__ == "__main__":
    import sys
    print(sys.argv[1])
    plot_simulation_topology(sys.argv[1])
    pass
