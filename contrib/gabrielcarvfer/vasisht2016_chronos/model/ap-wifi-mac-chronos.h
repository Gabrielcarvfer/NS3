...
#ifndef AP_WIFI_MAC_CHRONOS_H
#define AP_WIFI_MAC_CHRONOS_H
 ...
 */
    class ApWifiMacChronos : public RegularWifiMac
    {
    public:
        /**
    ...
    private:
        ...
        unsigned m_activeNetwork;
        unsigned m_activeNetworkPrevious;
        ...
        typedef struct {
            float coordinates[2];
            float distance;
            //Array containing indexes of sta_antennas ap_antennas and distance between them
            std::map<std::pair<unsigned,unsigned>, antenna_data_t> antenna_pairs;
        }ChronosLocationData;

        //Array containing indexes and distance between AP own antennas
        std::vector<antenna_data_t> ap_antennas_distances;
        std::map<Mac48Address, ChronosLocationData> chronosLocationData;

        void ChronosTracking();
        float euclidian_distance(float a[2], float b[2]);

        void calculate_location(ChronosLocationData* sta_antennas,
