...
int main(int argc, char *argv[])
{
    ...
    bool g_verbose = true;
    bool trace = false;
    int simulationDuration = 15;
    double startApplication = 1.0, endApplication = 20.0;
    double beaconInterval = 0.8, packetInterval = 100.0, maxBeaconInterval = 6.4, minBeaconInterval = 0.1;
...
// 1. Create the nodes and hold them in a container
    NodeContainer wifiStaticStaNodes, wifiMovingStaNodes, wifiApNodes, wifiStaticSta2Nodes;
    wifiStaticStaNodes.Create(0);
    wifiMovingStaNodes.Create(1);
    wifiApNodes.Create(nAPs);
    wifiStaticSta2Nodes.Create(0);
...
//NS_OBJECT_ENSURE_REGISTERED(StaWifiMac);
    ...
    NetDeviceContainer staticStaDevices, movingStaDevices, wifiStaticSta2Devices;

    staticStaDevices = wifi.Install(phy, mac, wifiStaticStaNodes);
    movingStaDevices = wifi.Install(phy, mac, wifiMovingStaNodes);
    wifiStaticSta2Devices = wifi.Install(phy, mac, wifiStaticSta2Nodes);
...
// 4. Set mobility of the nodes
    ...
    // Configure mobility of APs
        mobility.SetPositionAllocator ("ns3::RandomDiscPositionAllocator",
                                       "X", StringValue ("300.0"),
                                       "Y", StringValue ("300.0"),
                                       "Rho", StringValue("ns3::UniformRandomVariable[Min=-5|Max=5]"));
    ...
    mobility.Install(wifiStaticSta2Nodes);
...
//
    ...
    for (int i = 0; i < wifiMovingStaNodes.GetN(); i++)
    {
        ...
        mob->SetVelocity(Vector(25.0, 45.0, 0.0));
    }
...
// 6.Add Internet layers stack
    ...
    stack.Install(wifiStaticSta2Nodes);

// 7. Assign IP address to each device
    Ipv4AddressHelper address;
    Ipv4InterfaceContainer wifiStaticStaInterfaces, wifiMovingStaInterfaces, wifiApInterfaces, wifiStaticSta2Interfaces;
    address.SetBase("10.1.1.0", "255.255.255.0");
    wifiApInterfaces.Add(address.Assign(apDevices.Get(0)));
    ...
    //wifiApInterfaces.Add(address.Assign(apDevices.Get(1)));

    //address.SetBase("10.1.2.0", "255.255.255.0");
    wifiApInterfaces.Add(address.Assign(apDevices.Get(1)));
    //wifiStaticSta2Interfaces.Add(address.Assign(wifiStaticSta2Devices));
    //Simulator::Schedule(Seconds(10), &changeNetwork, wifiMovingStaNodes.Get(0), wifiApNodes.Get(0), wifiApNodes.Get(1));
...
// 8a. Create and setup applications (traffic sink)
    ...
    //serverApps.Add(echoServer.Install(wifiApNodes.Get(1)));
...
// 8b. Create and setup applications (traffic source)
    ...
    UdpEchoClientHelper echoClient1(wifiApNodes.Get(1)->GetObject<Ipv4>()->GetAddress(1,0).GetLocal(), 8080);
    echoClient1.SetAttribute("MaxPackets", UintegerValue(maxpackets));
    echoClient1.SetAttribute("Interval", TimeValue(MilliSeconds(packetInterval)));
    echoClient1.SetAttribute("PacketSize", UintegerValue(packetSize));

    ApplicationContainer staticSta2ClientApp = echoClient1.Install(wifiStaticStaNodes);
    staticSta2ClientApp.Start(Seconds(startApplication));
    staticSta2ClientApp.Stop(Seconds(endApplication));
