...
/*  printkunc recieves a pointer to the key of a node and prints it. */
...
void InorderTreePrint(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  if (x != tree->nil) {
    ...
    tree->PrintKey(x->key);
    printk("  l->key=");
    if( x->left == nil) printk("NULL"); else tree->PrintKey(x->left->key);
    printk("  r->key=");
    if( x->right == nil) printk("NULL"); else tree->PrintKey(x->right->key);
    printk("  p->key=");
    if( x->parent == root) printk("NULL"); else tree->PrintKey(x->parent->key);
    printk("  red=%i\n",x->red);
...
/*    OUTPUT:  none */
/**/
/*    EFFECT:  Destroys the key and kfrees memory */
/**/
/*    Modifies Input: tree */
/**/
/***********************************************************************/
...
/*    OUTPUT:  none */
/**/
/*    EFFECT:  Deletes z from tree and kfrees the key and info of z */
