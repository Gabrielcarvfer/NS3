...
#include "ns3/names.h"
#include "ns3/udp-echo-client2.h"
...
UdpEchoClientHelper::InstallPriv (Ptr<Node> node) const
{
  ...
  return app;
    ...
    UdpEchoClient2Helper::UdpEchoClient2Helper (Address address, uint16_t port)
    {
      m_factory.SetTypeId (UdpEchoClient2::GetTypeId ());
      SetAttribute ("RemoteAddress", AddressValue (address));
      SetAttribute ("RemotePort", UintegerValue (port));
    }

    UdpEchoClient2Helper::UdpEchoClient2Helper (Address address)
    {
      m_factory.SetTypeId (UdpEchoClient2::GetTypeId ());
      SetAttribute ("RemoteAddress", AddressValue (address));
    }
    ...
    void
    UdpEchoClient2Helper::SetAttribute (
            std::string name,
            const AttributeValue &value)
    {
      m_factory.Set (name, value);
    ...
    void
    UdpEchoClient2Helper::SetFill (Ptr<Application> app, std::string fill)
    {
      app->GetObject<UdpEchoClient2>()->SetFill (fill);
    }

    void
    UdpEchoClient2Helper::SetFill (Ptr<Application> app, uint8_t fill, uint32_t dataLength)
    {
      app->GetObject<UdpEchoClient2>()->SetFill (fill, dataLength);
    }

    void
    UdpEchoClient2Helper::SetFill (Ptr<Application> app, uint8_t *fill, uint32_t fillLength, uint32_t dataLength)
    {
      app->GetObject<UdpEchoClient>()->SetFill (fill, fillLength, dataLength);
    }

    ApplicationContainer
    UdpEchoClient2Helper::Install (Ptr<Node> node) const
    {
      return ApplicationContainer (InstallPriv (node));
    }

    ApplicationContainer
    UdpEchoClient2Helper::Install (std::string nodeName) const
    {
      Ptr<Node> node = Names::Find<Node> (nodeName);
      return ApplicationContainer (InstallPriv (node));
    }
    ...
    ApplicationContainer
    UdpEchoClient2Helper::Install (NodeContainer c) const
    {
      ApplicationContainer apps;
      for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
      {
    ...
    Ptr<Application>
    UdpEchoClient2Helper::InstallPriv (Ptr<Node> node) const
    {
      Ptr<Application> app = m_factory.Create<UdpEchoClient2> ();
      node->AddApplication (app);
