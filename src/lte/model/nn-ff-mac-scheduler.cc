...
#include <chrono>
#include "nn-ff-mac-scheduler.h"


namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("NnFfMacScheduler");
...
NS_OBJECT_ENSURE_REGISTERED (NnFfMacScheduler);
...
torch::jit::script::Module NnFfMacScheduler::nn_module;

NnFfMacScheduler::NnFfMacScheduler ()
  :   m_cschedSapUser (0),
    m_schedSapUser (0),
    m_timeWindow (99.0),
    m_nextRntiUl (0)
{
  ...
  m_cschedSapProvider = new MemberCschedSapProvider<NnFfMacScheduler> (this);
  m_schedSapProvider = new MemberSchedSapProvider<NnFfMacScheduler> (this);
  m_ffrSapProvider = 0;
  m_ffrSapUser = new MemberLteFfrSapUser<NnFfMacScheduler> (this);
  ...
  for (auto it2 = outputfiles.begin(); it2 != outputfiles.end(); it2++)
  {
      ...
      outputfileMap.insert({hashval, outputfile});

      auto it = outputfileMap.at(hash(*it2));
      if (*it2 == "dump_activeharqs.json")
          *it << "Time(%)" << "\t" << "RNTI" << "\t" << "LCID" << "\t" << "Harq" << "\n";
      else if (*it2 == "dump_rlcsched.json")
          *it << "Time(%)" << "\t" << "RNTI" << "\t" << "Flow" << "\t" << "Size" << "\n";
      else
          *it << "{";
          continue;

  }
  std::string model = PROJECT_SOURCE_PATH"/scratch/model.pt";
  nn_module = torch::jit::load(model);

  // Create a vector of inputs.
  std::vector<torch::jit::IValue> inputs;
  inputs.push_back(torch::zeros(160));
...
NnFfMacScheduler::DoDispose ()
{
  ...
  for (auto it = outputfileMap.begin(); it != outputfileMap.end(); it++)
  {
      ...
      {
          std::stringstream ss;
          auto filePtr = outputfileMap.at(hash("dump_lcidqci.json"));
          ss << "\"lcid_qci\":{";
...
TypeId
NnFfMacScheduler::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NnFfMacScheduler")
    .SetParent<FfMacScheduler> ()
    .SetGroupName("Lte")
    .AddConstructor<NnFfMacScheduler>()
    .AddAttribute ("CqiTimerThreshold",
                   "The number of TTIs a CQI is valid (default 1000 - 1 sec.)",
                   UintegerValue (1000),
                   MakeUintegerAccessor (&NnFfMacScheduler::m_cqiTimersThreshold),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("CqaMetric",
                   "NnFfMacScheduler metric type that can be: NnFf, CqaPf",
                   StringValue ("NnFf"),
                   MakeStringAccessor (&NnFfMacScheduler::m_CqaMetric),
                   MakeStringChecker ())
    .AddAttribute ("HarqEnabled",
                   "Activate/Deactivate the HARQ [by default is active].",
                   BooleanValue (true),
                   MakeBooleanAccessor (&NnFfMacScheduler::m_harqOn),
                   MakeBooleanChecker ())
    .AddAttribute ("UlGrantMcs",
                   "The MCS of the UL grant, must be [0..15] (default 0)",
                   UintegerValue (0),
                   MakeUintegerAccessor (&NnFfMacScheduler::m_ulGrantMcs),
...
void
NnFfMacScheduler::SetFfMacCschedSapUser (FfMacCschedSapUser* s)
{
  m_cschedSapUser = s;
}

void
NnFfMacScheduler::SetFfMacSchedSapUser (FfMacSchedSapUser* s)
{
  m_schedSapUser = s;
}

FfMacCschedSapProvider*
NnFfMacScheduler::GetFfMacCschedSapProvider ()
{
  return m_cschedSapProvider;
}

FfMacSchedSapProvider*
NnFfMacScheduler::GetFfMacSchedSapProvider ()
{
  return m_schedSapProvider;
}

void
NnFfMacScheduler::SetLteFfrSapProvider (LteFfrSapProvider* s)
{
  m_ffrSapProvider = s;
}

LteFfrSapUser*
NnFfMacScheduler::GetLteFfrSapUser ()
{
  return m_ffrSapUser;
}

void
NnFfMacScheduler::DoCschedCellConfigReq (const struct FfMacCschedSapProvider::CschedCellConfigReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
NnFfMacScheduler::DoCschedUeConfigReq (const struct FfMacCschedSapProvider::CschedUeConfigReqParameters& params)
{
...
void
NnFfMacScheduler::DoCschedLcConfigReq (const struct FfMacCschedSapProvider::CschedLcConfigReqParameters& params)
{
...
void
NnFfMacScheduler::DoCschedLcReleaseReq (const struct FfMacCschedSapProvider::CschedLcReleaseReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
NnFfMacScheduler::DoCschedUeReleaseReq (const struct FfMacCschedSapProvider::CschedUeReleaseReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
NnFfMacScheduler::DoSchedDlRlcBufferReq (const struct FfMacSchedSapProvider::SchedDlRlcBufferReqParameters& params)
{
...
void
NnFfMacScheduler::DoSchedDlPagingBufferReq (const struct FfMacSchedSapProvider::SchedDlPagingBufferReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
NnFfMacScheduler::DoSchedDlMacBufferReq (const struct FfMacSchedSapProvider::SchedDlMacBufferReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  NS_FATAL_ERROR ("method not implemented");
  return;
...
int
NnFfMacScheduler::GetRbgSize (int dlbandwidth)
{
  //if (dlbandwidth == 100)
  //    return 2;
...
unsigned int
NnFfMacScheduler::LcActivePerFlow (uint16_t rnti)
{
  std::map <LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
...
uint8_t
NnFfMacScheduler::HarqProcessAvailability (uint16_t rnti)
{
...
uint8_t
NnFfMacScheduler::UpdateHarqProcessId (uint16_t rnti)
{
  NS_LOG_FUNCTION (this << rnti);
...
void
NnFfMacScheduler::RefreshHarqProcesses ()
{
  NS_LOG_FUNCTION (this);
...
void
NnFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  std::chrono::high_resolution_clock::time_point t_enc, t_proc;
  ...
  typedef struct SchedReg schedReg;

  std::vector<schedReg> schedRegistry;
  std::map<int,int>               ueList;
  std::map<int,std::vector<int>> cqiList;

  std::map<int,bool> gbrQci{{1,true},{2,true},{3,true},{4,true},{65,true},{66,true},{75,true}};
  int maxBitsToTransmit = 0;
  int numReqs = 10;
  int numSlots = 24 / rbgSize;

  //1 Prepare input for the NN
  //1.1 Get a list of LCIDs, related QCI, GBR and bits to transmit, plus related UE and its CQI
  for (auto it = UeToAmountOfDataToTransfer.begin(); it != UeToAmountOfDataToTransfer.end(); it++)
  {
      int rnti = it->first.m_rnti;
      //Look for CQI list for UEs
      auto it2 = cqiList.find(rnti);
      if (it2 == cqiList.end())
      {
          std::vector<int> ueCqi(numSlots);
          //Assemble list for UE CQI
          for (auto slotIt : availableRBGs)
          {
            ueCqi.at(slotIt) = m_a30CqiRxed.at(rnti).m_higherLayerSelected.at(slotIt).m_sbCqi.at(0);
          }
          cqiList.emplace(it->first.m_rnti, ueCqi);
      }
      ueList.emplace(rnti, rnti);
      schedRegistry.push_back(schedReg{rnti, it->first.m_lcId, m_lcidQci.at(it->first.m_lcId).qci, it->second, cqiList.at(rnti)});
      if (it->second > maxBitsToTransmit)
          maxBitsToTransmit = it->second;
  }


  int prevNumAvailableSlots = availableRBGs.size();

  //If theres something to schedule
  if (schedRegistry.size() > 0)
  {
      //1.2 Assign positions to ues, to recover from encoding
      int i = 0;
      std::map<int, int> positionUeMap;
      std::map<int, int> uePositionMap;
      for (auto ueIt = ueList.begin(); ueIt != ueList.end(); ueIt++)
      {
          positionUeMap.emplace(i, ueIt->first);
          uePositionMap.emplace(ueIt->first, i);
          i++;
      }

      //1.3 Consolidate data into an encoded array
      std::map<int, float> normalizedQci;
      std::vector<int> qciPriority{69, 65, 5, 1, 66, 75, 3, 2, 4, 70, 6, 79, 7, 8, 9};
      i = 0;
      for (auto qciIt = qciPriority.rbegin(); qciIt != qciPriority.rend(); qciIt++)
      {
          normalizedQci.emplace(*qciIt, (float) ((float)i / (float) qciPriority.size()) );
          i++;
      }

      std::vector<float> normalizedCqi;
      for (i = 0; i < 16; i++)
      {
          normalizedCqi.push_back( (float) ((float)i / (float) 15 ) );
      }

      //Encode data
      std::vector<float> encodedData;
      for (auto regIt = schedRegistry.begin(); regIt != schedRegistry.end(); regIt++)
      {
          float normUe = (float)uePositionMap.find(regIt->ue)->second / (float) uePositionMap.size();
          float normQci = normalizedQci.find(regIt->qci)->second;
          float normBitsToTransmit = (float)regIt->bitsToTransmit /(float)maxBitsToTransmit;

          float gbr = 0.0;
          {
              auto t = gbrQci.find(regIt->qci);
              if (t != gbrQci.end())
                  gbr = 1.0;
          }

          encodedData.push_back(normUe);
          encodedData.push_back(normQci);
          encodedData.push_back(normBitsToTransmit);
          encodedData.push_back(gbr);

          for (auto t = regIt->cqi.begin(); t != regIt->cqi.end(); t++)
              encodedData.push_back(normalizedCqi.at(*t));

      }

      //1.4 fill with zeros
      int fillingZeroes = 160 - encodedData.size();
      while(fillingZeroes > 0)
      {
          encodedData.push_back(0.0);
          fillingZeroes--;
      }

      //2 Consolidate input into a tensor
      at::Tensor f = torch::tensor(encodedData);

      std::vector<torch::jit::IValue> inputs;
      inputs.push_back(f);

      t_enc = std::chrono::high_resolution_clock::now();

      //3 Execute the model and turn its output into a tensor.
      at::Tensor output = nn_module.forward(inputs).toTensor();

      t_proc = std::chrono::high_resolution_clock::now();

      //4 Decode NN output into the information to assemble the resulting schedule
      std::vector<int> reqIndex;
      std::vector<float> reqVal;
      for (int slot = 0; slot < numSlots; slot++)
      {
          auto slotSlice = output.slice(0, slot * numReqs, (slot + 1) * numReqs);
          int index = slotSlice.argmax().item<int>();
          float val = slotSlice.max().item<float>();
          reqIndex.push_back(index);
          reqVal.push_back(val);
      }

      //5 Assemble schedule using output of NN
      for (int slot = 0; slot < numSlots; slot++)
      {
          //Skip occupied RBGs
          if (availableRBGs.find(slot) == availableRBGs.end())
              continue;

          //Fetch rnti for current slot
          int schedRegIndex = reqIndex[slot];

          // If index is out of bounds, skip and leave slot free
          if ( schedRegIndex >= schedRegistry.size() )
              continue;

          schedReg currReg = schedRegistry[schedRegIndex];

          // If nothing to transmit, skip
          if ( currReg.bitsToTransmit == 0)
              continue;

          int rnti = currReg.ue;
          int lcid = currReg.lcid;

          // Check if HARQ process is available. If none is available, skip
          if ( !HarqProcessAvailability(rnti) )
              continue;

          qos_rb_and_CQI_assigned_to_lc s;
          s.cqi_value_for_lc = currReg.cqi.at(slot);
          s.resource_block_index = slot;

          //Search for allocation map entry
          itMap = allocationMapPerRntiPerLCId.find(rnti);

          //If rnti not in allocation map, create an entry and register the current block
          if (itMap == allocationMapPerRntiPerLCId.end())
          {
            ...
            tempMap.insert(std::pair<uint8_t, qos_rb_and_CQI_assigned_to_lc>(lcid, s));
            allocationMapPerRntiPerLCId.insert(std::pair<uint16_t, std::multimap<uint8_t, qos_rb_and_CQI_assigned_to_lc> >(rnti, tempMap));
          }
          else
          {
            itMap->second.insert(std::pair<uint8_t, qos_rb_and_CQI_assigned_to_lc>(lcid, s));
          }

          // erase current RBG from the list of available RBG
          availableRBGs.erase(slot);
      }
  }

  std::chrono::high_resolution_clock::time_point t_end = std::chrono::high_resolution_clock::now();
  //std::cout << "NN enc took " << std::chrono::duration_cast<std::chrono::microseconds>( t_enc - t_begin ).count() << " us" << "\n";
  //std::cout << "NN proc took " << std::chrono::duration_cast<std::chrono::microseconds>( t_proc - t_enc ).count() << " us" << "\n";
  //std::cout << "NN dec took " << std::chrono::duration_cast<std::chrono::microseconds>( t_end - t_proc ).count() << " us" << "\n";
  //std::cout << "NN (enc+proc+dec) scheduling took " << std::chrono::duration_cast<std::chrono::microseconds>( t_end - t_begin ).count() << " us" << std::endl;

  if ( prevNumAvailableSlots != availableRBGs.size() )
  {
      //std::cout << prevNumAvailableSlots - availableRBGs.size() << " RBGs were allocated with NN scheduler out of " << prevNumAvailableSlots << std::endl;
  }
  else
  {
      //std::cout << "No RBGs were allocated with NN scheduler out of " << prevNumAvailableSlots << std::endl;
  }
...
*/
  ...
  //std::cout << this << " Sim " << Simulator::Now().GetMilliSeconds() << "ms Allocated " << count_allocated_resource_blocks << " RBs" << std::endl;
  return;
}

void
NnFfMacScheduler::DoSchedDlRachInfoReq (const struct FfMacSchedSapProvider::SchedDlRachInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
NnFfMacScheduler::DoSchedDlCqiInfoReq (const struct FfMacSchedSapProvider::SchedDlCqiInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
double
NnFfMacScheduler::EstimateUlSinr (uint16_t rnti, uint16_t rb)
{
...
void
NnFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
...
void
NnFfMacScheduler::DoSchedUlNoiseInterferenceReq (const struct FfMacSchedSapProvider::SchedUlNoiseInterferenceReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  return;
}
...
void
NnFfMacScheduler::DoSchedUlSrInfoReq (const struct FfMacSchedSapProvider::SchedUlSrInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  return;
}
...
void
NnFfMacScheduler::DoSchedUlMacCtrlInfoReq (const struct FfMacSchedSapProvider::SchedUlMacCtrlInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
NnFfMacScheduler::DoSchedUlCqiInfoReq (const struct FfMacSchedSapProvider::SchedUlCqiInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
NnFfMacScheduler::RefreshDlCqiMaps (void)
{
...
void
NnFfMacScheduler::RefreshUlCqiMaps (void)
{
  // refresh UL CQI  Map
...
void
NnFfMacScheduler::UpdateDlRlcBufferInfo (uint16_t rnti, uint8_t lcid, uint16_t size)
{
  std::map<LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
...
void
NnFfMacScheduler::UpdateUlRlcBufferInfo (uint16_t rnti, uint16_t size)
{
...
void
NnFfMacScheduler::TransmissionModeConfigurationUpdate (uint16_t rnti, uint8_t txMode)
{
