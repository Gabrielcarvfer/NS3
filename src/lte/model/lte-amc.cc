...
int
LteAmc::GetPrbSizeFromMcsAndNumerology(int mcs, int num)
{
    if (!prbDataLoaded)
        LoadPrbData ();
    std::tuple<uint8_t, uint8_t> key{(uint8_t)mcs, (uint8_t)num};
...
LteAmc::CreateCqiFeedbacks (const SpectrumValue& sinr, uint8_t rbgSize)
{
  ...
  if (m_amcModel == PiroEW2010)
  ...
  else if (m_amcModel == MiErrorModel  || m_amcModel == MiesmErrorModel)
    {
      ...
      std::vector <int> rbgMap(sinr.ConstValuesEnd()-sinr.ConstValuesBegin(), 0);
      ...
      //std::cout << "snr " << 10*log10(sinr.ValuesAt(0)) << std::endl; //enable this to collect calculated SNR
      if (mcs > 0)
      {
          mcs--;
      }
      //std::cout << Simulator::Now().GetSeconds() << "  generateCqiFeedback " << 10*log10(sinr[0]) << " tbler " << tbStats.tbler << std::endl;
